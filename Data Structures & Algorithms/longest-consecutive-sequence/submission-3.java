class Solution {
    ...
    public int longestConsecutive(int[] nums) {
        ...
        // last time i tried map where it was highest -> lowest
        // when I see a bunch of stuff where you can assemble strings of sets, 
        //  its probably a union find but thats pretty tough to implement but i think its o(n) amortized

        // better way is because the length of nums is 1K, ride off of that
        // load everything into a hash set and use a walk from start
        // todo wtf is walk from start

        // set of every unique number in nums
        ...
        int longestConsecutiveSeen = 0;

        for (Integer num : nums) {

            int currentConsecutive = 1;
            int numToCheckFor = num - 1;
            while (uniqueNums.contains(numToCheckFor)) {
                ++currentConsecutive;
                --numToCheckFor;
            }

            longestConsecutiveSeen = Math.max(longestConsecutiveSeen, currentConsecutive);
        }

        return longestConsecutiveSeen;
    }
